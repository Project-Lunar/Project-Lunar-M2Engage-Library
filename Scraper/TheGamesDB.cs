...
namespace ProjectLunarUI
{
    public class TheGamesDB : IScraper
    {
        ...
        public TheGamesDB()
        {
            allPublishers = JObject.Parse(Scraper.Properties.Resources.PublisherJson);
        ...
        public List<ScraperData> GetGameInformation(string gameName, GameSystems system)
        {
            ...
            using (HttpClient httpClient = new HttpClient())
            {
                ...
                string gamesDbRequest = $"https://api.thegamesdb.net/v1/Games/ByGameName?apikey={apiKey}&name={gameName}&fields=players%2Cpublishers%2Cgenres%2Coverview%2Cplatform&filter%5Bplatform%5D={platform}&include=boxart%2Cplatform";
            ...
            foreach (var game in scrapedGames["data"]["games"])
            {
                ...
                string platformName = GetPlatformName(platformNum);//(platformNum == 18 ? "Sega Genesis" : (platformNum == 36 ? "Sega Mega Drive" : (platformNum == 35 ? "Sega Master System" : "unknown")));
        ...
        private static string GetPlatformId(string platformName)
        {
            JObject platforms = JObject.Parse(Scraper.Properties.Resources.TGDBPlatformJson);
            string platformId = platforms["data"]["platforms"].Children().Children().Where(c => c["name"].ToString().Equals(platformName))
                                                                                    .FirstOrDefault()["id"].ToString();
            return platformId;
        }

        private string GetPlatformName(int platformNum)
        {
            JObject platforms = JObject.Parse(Scraper.Properties.Resources.TGDBPlatformJson);
            string platformId = platforms["data"]["platforms"].Children().Children().Where(c => ((int)c["id"]).Equals(platformNum))
