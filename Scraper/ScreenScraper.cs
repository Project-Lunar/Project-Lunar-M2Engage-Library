    ...
    public class ScreenScraper : IScraper
    {
        ...
        public List<ScraperData> GetGameInformation(string MD5, string SHA1, string romName, GameSystems system, string romTitle = null)
        {
            ...
            using (HttpClient httpClient = new HttpClient())
            {
                ...
                StringReader cdata = new StringReader(Encoding.Default.GetString(Scraper.Properties.Resources.ss));
                string devId = cdata.ReadLine();
                string devPwd = cdata.ReadLine();
                string ssId = cdata.ReadLine();
                string ssPwd = cdata.ReadLine();
                string gamesDbRequest = $"https://www.screenscraper.fr/api/jeuInfos.php?devid={devId}&devpassword={devPwd}&softname=ProjectLunar1.0&ssid={ssId}&sspassword={ssPwd}&output=json&md5={MD5}&sha1={SHA1}&systemeid={platform}";
                ...
                if (responseJson.ToLower().Contains("erreur") && !responseJson.ToLower().Contains("header") && !responseJson.ToLower().Contains("response"))
                {
                    ...
                    gamesDbRequest = $"https://www.screenscraper.fr/api/jeuInfos.php?devid={devId}&devpassword={devPwd}&softname=ProjectLunar1.0&ssid={ssId}&sspassword={ssPwd}&output=json&romnom={romName}&systemeid={platform}";
                    responseJson = httpClient.GetStringAsync(new Uri(gamesDbRequest)).Result;
                }
                ...
                if (responseJson.ToLower().Contains("erreur") && !responseJson.ToLower().Contains("header") && !responseJson.ToLower().Contains("response"))
                {
                    ...
                    gamesDbRequest = $"https://www.screenscraper.fr/api/jeuInfos.php?devid={devId}&devpassword={devPwd}&softname=ProjectLunar1.0&ssid={ssId}&sspassword={ssPwd}&output=json&romnom={gameName}&systemeid={platform}";
                    responseJson = httpClient.GetStringAsync(new Uri(gamesDbRequest)).Result;
                }
                if (responseJson.ToLower().Contains("erreur") && !responseJson.ToLower().Contains("header") && !responseJson.ToLower().Contains("response"))
                {
                    //Try using the typed title as a ROM name
                    gamesDbRequest = $"https://www.screenscraper.fr/api/jeuInfos.php?devid={devId}&devpassword={devPwd}&softname=ProjectLunar1.0&ssid={ssId}&sspassword={ssPwd}&output=json&romnom={romTitle}&systemeid={platform}";
                    responseJson = httpClient.GetStringAsync(new Uri(gamesDbRequest)).Result;
                }
