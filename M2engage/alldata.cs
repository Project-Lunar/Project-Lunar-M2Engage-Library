...
namespace ProjectLunarUI.M2engage
{
    ...
    {
        public const int SRAM_Save_Count = 255;
        ...
        public static string AddGameToRomsObject(string romCode, JObject romsObject, string romPath, string basePath, string sysRegion, string country, bool use6ButtonHack)
        {
            ...
            string RomName = CleanupRomName(romPath);

            //In case of duplicate rom...
            if (!RomName.Equals("executor"))
            {
        ...
        public static string CleanupRomName(string romPath)
        {
            string RomName = Path.GetFileNameWithoutExtension(romPath).Replace(" ", "_").Replace("'", "").Replace("\"", "")
                                                                      .Replace(".", "").Replace("!", "");
        ...
        public static string AddGameToTitlesObject(int newGameTime, string specialRomCode, JObject titlesObject, JObject texturesObject, string titleName, string description, string copyright, string dev_name, string sysRegion, int demoTime, ScraperData gameData = null, string romPath = null)
        {
            ...
            int maxPosition = titlesObject["items"].Max(c => ((int)c["tposition"]));
            newGame["tposition"] = maxPosition + 1;
            ...
            if (string.IsNullOrEmpty(specialRomCode))
            {
                ...
                romCode = crc.ToString("X8").Substring(0, 7); //It's weird, but only 7 characters allowed... 😕
            }
            else
            {
        ...
        public static string DeleteGameFromRomsObject(string romCode, JObject romsObject, string basePath, string romsJsonPath,
                                              MArchivePacker packer, List<string> touchedFiles)
        {
            JToken romEntry = romsObject["root"]["m2epi"]["version"][romCode];
            string romName = romEntry["rom"].ToString();
            int romCount = romsObject["root"]["m2epi"]["version"].Children().Children().Where(c => c["rom"].ToString().Equals(romName)).Count();
            ...
            if (romCount.Equals(1)) //prevents deleting executor rom until it's the last game using it.
            {
        ...
        public static void ModeTitleSelectNutPatch(string basePath, MArchivePacker packer, List<string> touchedFiles)
        {
            ...
            //find first patching point (line 46)
            string line = string.Empty;
            while (true)
            {
                line = modeTitleSelectNut.ReadLine();
                newNutScript.AppendLine(line);
                if (line.Trim().Equals("const PKGMOTSMOOTHING = 1;"))
                {
                    break;
                }
            }
            ...
            //Find next block...
            line = LoopUntilFind("for ( i = 0; i < 50 ; i++ )", modeTitleSelectNut, newNutScript);
            newNutScript.AppendLine(line.Replace("50", "MAX_TITLES"));

            //Find next block...
            line = LoopUntilFind("s_indextable.append(49);", modeTitleSelectNut, newNutScript);
            newNutScript.AppendLine(line.Replace("49", "MAX_TITLES - 1"));

            //skip another and loop one for each language
            for (int i = 0; i < 8; i++)
            {
                line = LoopUntilFind("indexoffset = 50", modeTitleSelectNut, newNutScript);
                newNutScript.AppendLine(line.Replace("50", "MAX_TITLES"));
            }

            //skip and last modification...

            line = LoopUntilFind("for ( id = 0; id < 50; id++ )", modeTitleSelectNut, newNutScript);
            newNutScript.AppendLine(line.Replace("50", "MAX_TITLES"));
        ...
        private static string LoopUntilFind(string valueToFind, StringReader modeTitleSelectNut, StringBuilder newNutScript)
        {
            string line;
            while (true)
            {
                line = modeTitleSelectNut.ReadLine();
                if (line.Trim().StartsWith(valueToFind))
                {
                    break;
                }
                newNutScript.AppendLine(line);
        ...
        public static void TitleModeTopUpgrade(string basePath, MArchivePacker packer, List<string> touchedFiles, string dev_id = null)
        {
            if (string.IsNullOrEmpty(dev_id))
            {
                string jsonPath = PrepareAccess($@"{basePath}\system\config\system_prof.psb.m", packer, touchedFiles);
                string systemJson = File.ReadAllText(jsonPath);
            ...
            string titlesJsonPath = PrepareAccess($@"{basePath}\{dev_id}\config\title_mode_top.psb.m", packer, touchedFiles);
            string titlesJson = File.ReadAllText(titlesJsonPath);
            JObject titlesObject = JObject.Parse(titlesJson);

            if (titlesObject["items"][0]["tposition"] == null)
            {
                int totalItems = titlesObject["items"].Count();
                int gameCount = totalItems / 8;
                int i = 0;
                foreach (var titleObj in titlesObject["items"])
                {
                    ((JObject)titleObj).Add("tposition", i++ % gameCount);
                }
                File.WriteAllText(titlesJsonPath, titlesObject.ToString());
            }
        }

        public static void TitleProfUpgrade(string basePath, MArchivePacker packer, List<string> touchedFiles, string dev_id = null)
        {
            if (string.IsNullOrEmpty(dev_id))
            {
                string jsonPath = PrepareAccess($@"{basePath}\system\config\system_prof.psb.m", packer, touchedFiles);
            ...
            string romsJsonPath = PrepareAccess($@"{basePath}\{dev_id}\config\title_prof.psb.m", packer, touchedFiles);
            ...
            if (gvs[0].Count() < 4)
            {
                gvs.OrderBy(c => (int)c[0]).ToList().ForEach(c => c[2].AddAfterSelf(c[0]));
            }
            File.WriteAllText(romsJsonPath, romsObject.ToString());
        }
        ...
        public static void ModeTitleSelectNutAddUpdate(string basePath, MArchivePacker packer, List<string> touchedFiles, bool addGame = true)
        {
            ...
            //skip first 46 lines
            string line = LoopUntilFind("const MAX_TITLES", modeTitleSelectNut, newNutScript);

            string maxTitlesConst = line; //47
            ...
            line = LoopUntilFind("const TITLE_NUM_JP", modeTitleSelectNut, newNutScript);
            string titleNumJp = line; //526
            ...
            line = LoopUntilFind("const TITLE_FRONT_NUM_Y_JP", modeTitleSelectNut, newNutScript);
            string frontNumYjp = line; //529
            ...
            line = LoopUntilFind("const TITLE_SIDE_NUM_Y_JP", modeTitleSelectNut, newNutScript);
            string sideNumYjp = line; //531
            ...
            line = LoopUntilFind("const TITLE_NUM_US", modeTitleSelectNut, newNutScript);
            newNutScript.AppendLine(line.Replace(titleNum.ToString(), (titleNum + modifier).ToString())); //533
            line = LoopUntilFind("const TITLE_FRONT_NUM_Y_US", modeTitleSelectNut, newNutScript);
            newNutScript.AppendLine(line.Replace(frontNumY.ToString(), (rowCountFront).ToString()));
            line = LoopUntilFind("const TITLE_SIDE_NUM_Y_US", modeTitleSelectNut, newNutScript);
            newNutScript.AppendLine(line.Replace(sideNumY.ToString(), (rowCountSide).ToString()));

            line = LoopUntilFind("const TITLE_NUM_EU", modeTitleSelectNut, newNutScript);
            newNutScript.AppendLine(line.Replace(titleNum.ToString(), (titleNum + modifier).ToString())); //537
            line = LoopUntilFind("const TITLE_FRONT_NUM_Y_EU", modeTitleSelectNut, newNutScript);
            newNutScript.AppendLine(line.Replace(frontNumY.ToString(), (rowCountFront).ToString()));
            line = LoopUntilFind("const TITLE_SIDE_NUM_Y_EU", modeTitleSelectNut, newNutScript);
            newNutScript.AppendLine(line.Replace(sideNumY.ToString(), (rowCountSide).ToString()));

            line = LoopUntilFind("const TITLE_NUM_AS", modeTitleSelectNut, newNutScript);
            newNutScript.AppendLine(line.Replace(titleNum.ToString(), (titleNum + modifier).ToString())); //541
            line = LoopUntilFind("const TITLE_FRONT_NUM_Y_AS", modeTitleSelectNut, newNutScript);
            newNutScript.AppendLine(line.Replace(frontNumY.ToString(), (rowCountFront).ToString()));
            line = LoopUntilFind("const TITLE_SIDE_NUM_Y_AS", modeTitleSelectNut, newNutScript);
            newNutScript.AppendLine(line.Replace(sideNumY.ToString(), (rowCountSide).ToString()));
